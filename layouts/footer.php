<footer class="bg-gray-900 text-white py-8">
    <div class="container mx-auto text-center">
        ...
        <div class="flex justify-center space-x-6 mb-4">
            <a href="https://github.com/nasywahvdr" target="_blank"
            ...
            <a href="mailto:vadriannasywah@gmail.com" target="_blank" class="text-gray-400 hover:text-white text-2xl">
...
<script>
    ...
    let index = 0;
    const itemsPerView = 3; // Menampilkan 3 card per slide
    const totalSlides = Math.ceil(items.length / itemsPerView);
    const itemMargin = 16; // Sesuaikan dengan gap-x-4 di Tailwind
    const itemWidth = items[0].clientWidth + itemMargin; // Pastikan width+margin dihitung
    const scrollStep = itemWidth * itemsPerView; // Geser sebanyak 3 card
    const dots = [];

    // Buat navigasi dot
    for (let i = 0; i < totalSlides; i++) {
        const dot = document.createElement('span');
        dot.classList = "dot w-3 h-3 bg-gray-400 rounded-full cursor-pointer transition hover:bg-gray-300";
    ...
    function updateCarousel() {
        carousel.style.transform = `translateX(-${index * scrollStep}px)`;
        updateDots();
    }
    ...
    nextBtn.addEventListener('click', () => {
        if (index < totalSlides - 1) {
            ...
            index = 0; // Kembali ke awal jika sudah mentok
        }
        updateCarousel();
    });
    ...
    prevBtn.addEventListener('click', () => {
        if (index > 0) {
            ...
            index = totalSlides - 1; // Lompat ke slide terakhir
        }
        updateCarousel();
    });
    ...
    // Auto-scroll setiap 5 detik (opsional)
    setInterval(() => {
        nextBtn.click();
    }, 5000);
</script>
...
<script>
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener("click", function (e) {
            ...
            if (this.hasAttribute("data-close-menu")) {
